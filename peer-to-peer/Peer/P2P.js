...
function start() {
	...
	navigator.getUserMedia({audio:false, video:true}, successCallback,
		function(error) {
...
function call() {
	...
	// Log info about video and audio device in use
		if (localStream.getVideoTracks().length > 0) {
			log('Utilizando dispositivo de video: ' + localStream.getVideoTracks()[0].label);
		}
		if (localStream.getAudioTracks().length > 0) {
			log('Utilizando dispositivo de audio: ' + localStream.getAudioTracks()[0].label);
		}
	}
	...
	log("RTCPeerConnection objeto: " + RTCPeerConnection);
